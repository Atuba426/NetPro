...
export default function viewProfilePage({ userProfile }) {
  ...
  const [connectionNull,setconnectionNull]= useState(true);
  const getUserPosts = async () => {
    const response = await dispatch(
      getConnectionRequests({ token: localStorage.getItem("token") })
    );
    console.log("getConnectionRequests result", response);
    await dispatch(getAllPost());
    await dispatch(
      getConnectionRequests({ token: localStorage.getItem("token") })
    );
    await dispatch(getAllMyConnectionRequests({token:localStorage.getItem("token")}))
  };
  ...
  useEffect(() => {
    ...
    if (
      ...
      setcurrentuserInConnection(true);
      if(authState.connections.find(user=>user.userId._id===userProfile.userId._id).status_accepted===true){
        setconnectionNull(false)
      }
    }
              ...
              <div style={{ flex: "0.8" }}>
                ...
                <div style={{display:"flex",alignItems:"center", gap:"1.2rem"}}>
                  {CurrentuserInConnection ?
                   <button  className={styles.connected_Button}>{connectionNull ? "Pending" : "Connected"}</button>
                  :
                    <button
                      onClick={() => {
                        dispatch(
            ...
            userProfile.pastWork.map((work,index)=>{
              return(
                <div key={index}className={styles.workHistoryCard}>
                  <p style={{display:"flex", alignItems:"center", fontWeight:"bold",gap:"0.8rem"}}>{work.company}-{work.position}</p>
                  <p>{work.years}</p>
                </div>
              )
            })
...
export async function getServerSideProps(context) {
  const request = await clientServer.get(
    "/getUser_And_Userprofile_BasedOn_Username",
    {
      params: {
